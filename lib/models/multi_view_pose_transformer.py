...
from core.loss import PerJointL1Loss, PerBoneL1Loss, PerProjectionL1Loss, PerProjectionL1Loss2D
...
class MultiviewPosetransformer(nn.Module):
    ...
    def __init__(self, backbone, cfg):
        ...
        ## Size: (number of joints (instances), model dimension (256))
        if self.query_embed_type == 'person_joint':  ## used
            ...
            # Only each joints are different
            # Each instance (query) has the same ins embedding
            self.joint_embedding = nn.Embedding(cfg.DECODER.num_keypoints,
                                    cfg.DECODER.d_model * 2)
            self.instance_embedding = nn.Embedding(1,
                                        cfg.DECODER.d_model * 2)
        elif self.query_embed_type == 'human_joint2':
            # Even joint & instance are the same
            self.joint_embedding = nn.Embedding(1,
                                    cfg.DECODER.d_model * 2)
            self.instance_embedding = nn.Embedding(1,
                                        cfg.DECODER.d_model * 2)
        elif self.query_embed_type == 'per_instance':
            # each instance has a different queries, while the joint is the same
            self.joint_embedding = nn.Embedding(1,
                                                cfg.DECODER.d_model * 2)
            self.instance_embedding = nn.Embedding(cfg.DECODER.num_instance,
                                                   cfg.DECODER.d_model * 2)
        ...
        matcher = HungarianMatcher(match_coord_est=cfg.DECODER.match_coord_est,
                                   match_coord_gt=cfg.DECODER.match_coord_gt,
                                   cost_class=2.,
                                   cost_pose=5.,
                                   method=cfg.DECODER.match_method,
                                   method_value=cfg.DECODER.match_method_value)
        ...
        weight_dict = {
            ...
            'loss_pose_perprojection_2d': cfg.DECODER.loss_pose_perprojection_2d,
            "loss_init": cfg.DECODER.loss_weight_init
        }
    ...
    ## views: input images nview of (batch, C, H, W)
    ## meta: store camera info, e.g., R, T, Intri
    def forward(self, views=None, meta=None):
        ...
        ## calculate camera rays for rayConv
        ...
        ## generating pose embeddings
        for lvl in range(nfeat_level):
            # this can be compute only once, without iterating over views
            if self.projattn_posembed_mode == 'use_rayconv': ## use_rayconv is used in config
                camera_rays.append(
        ...
        src_flatten_views = []
        mask_flatten_views = []  ## paddding after rayconv, see Line 161 projattn.py 
        ...
        if self.query_embed_type == 'person_joint':  ## used in config file
        ...
        ## batch: how many (multi-view paris) are processing together
        ## src: input to encoder
        ## tgt: input to the decoder
        query_embed, tgt = torch.split(query_embeds, c, dim=1)  # query_embeds: 150x512, c 256 -> 150x256 each
        query_embed = query_embed.unsqueeze(0).expand(batch, -1, -1)
        tgt = tgt.unsqueeze(0).expand(batch, -1, -1)

        ## make backbone features learnable
        # query adaptation
        if self.query_adaptation:  ## activated
            ...
            reference_points = self.reference_points(
                query_embed + ref_feats).sigmoid()  ## sigmoid to change to (0,1) normalized coordintates on image plane
        else:
            reference_points = self.reference_points(query_embed).sigmoid()  ## get ref points from embeddings
        ...
        ## layer-level update of the pose
        for lvl in range(hs.shape[0]):
            if lvl == 0:
                reference = init_reference
            else:
                reference = inter_references[lvl - 1]
            reference = inverse_sigmoid(reference) # change back to net output
            # mean after sigmoid
            ## class embedded: linear layer to map to 2 dim
            if self.pred_class_fuse == 'mean': ## used in config
            ...
            tmp = self.pose_embed[lvl](hs[lvl]) ## update one by one through each level
            tmp += reference
        ...
        # otherwise only ouput an estimation
        return out
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher,
                 weight_dict, losses, cfg, focal_alpha=0.25, root_idx=2, method='hungarian'):
        ...
        self.use_loss_pose_perprojection_2d = \
            cfg.DECODER.use_loss_pose_perprojection_2d
        ...
        if self.use_loss_pose_perprojection_2d:
            self.criterion_pose_perprojection_2d = \
                PerProjectionL1Loss2D(cfg.DECODER.loss_joint_type)
        ...
        self.use_ce_match = cfg.DECODER.use_ce_match # whether to use final outputs matching with gt as classification
                                    # instead of the anchors, only valid when using gt_match
    ...
    def loss_labels(self, outputs, meta, indices, num_samples, log=True):
        ...
        query_num = len(idx[0])
        target_classes_o = torch.ones((query_num,)).long().to(device)
        ...
        if log:
            ...
            losses['class_recall'] = accuracy(src_logits[idx],
                                                   target_classes_o, threshold=threshold)[0]
            losses['class_precision'] = precision(src_logits.squeeze(),
                                                   target_classes.squeeze(), threshold=threshold)[0]
        return losses
    ...
    def loss_poses(self, outputs, outputs_2d, 
                   meta, indices, num_samples=None, output_abs_coord=False):
        ...
        src_poses = outputs.view(bs, -1, num_joints, 3)[idx]
        ...
        if not self.criterion_pose_perjoint.loss_type == 'mpjpe':
            ...
            # TODO: Check why not use loss_cord.mean() directly!
        else:
        ...
        if self.use_loss_pose_perprojection_2d \
            and (outputs_2d is not None):
            num_views = outputs_2d.shape[1]
            src_poses_2d = outputs_2d.view(bs, num_views, -1, num_joints, 2)[idx[0],:,idx[1],...]
            ...
            for meta_iew in meta:
                camera = meta_iew['camera']
                for k,v in camera.items():
                    cameras[k].append(v[batch_ids])
            
            for k,v in cameras.items():
                cameras[k] = torch.stack(v,dim=1)
            ...
            if loss_pose_perprojection.item() > 1e5:
                loss_pose_perprojection = loss_pose_perprojection * 0.0
            losses['loss_pose_perprojection_2d'] = loss_pose_perprojection
    ...
    def get_loss(self, loss, outputs, outputs_2d, targets,
        ...
        if loss == 'joints':
            return \
                loss_map[loss](outputs, outputs_2d, targets, indices, num_samples, **kwargs)
        elif loss == 'labels':
            indices_ce = kwargs['indices_ce']
            kwargs.pop('indices_ce')
            return \
                loss_map[loss](outputs, targets, indices_ce, num_samples, **kwargs)
        else:
            return \
                loss_map[loss](outputs, targets, indices, num_samples, **kwargs)

    def forward(self, outputs, meta, outputs_origin=None):
        ...
        # construct an origin outputs matrix
        if outputs_origin: # gt match
            indices = self.matcher(outputs_origin, meta)
        else:
            indices = self.matcher(outputs_without_aux, meta)
            
            
        # A flag to use final matching for classification supervision
        if self.use_ce_match:
            if outputs_origin:
                indices_ce = self.matcher(outputs_without_aux, meta, method='hungarian-dis')
            else:
                indices_ce = indices
        else:
            # default 
        ...
        for loss in self.losses:
            kwargs = {}
            if loss == 'joints':
                # for estimation, use abs !
                kwargs["output_abs_coord"] = True
                losses.update(
                    self.get_loss(loss,
                                  outputs["pred_poses"]['outputs_coord'],
                                  outputs["pred_poses_2d"]['outputs_coord_2d'],
                                  meta, indices, num_samples, **kwargs))
            else:
                # if loss is label, add indices_ce
                if loss == 'labels':
                    kwargs["indices_ce"] = indices_ce
                losses.update(self.get_loss(loss, outputs, outputs, meta,
                                            indices, num_samples, **kwargs))
...
@torch.no_grad()
def precision(output, target, topk=(1,), threshold=0):
    ...
    preds_thresh = output_prob[...,1:2] > threshold
    pred = pred.logical_and(preds_thresh)
    pred = pred.squeeze(-1).long()  # batch_size, query_num

    # pred = pred.transpose(1,2)
    # correct = pred.eq(target.view(1, -1).expand_as(pred))

    pred_positive_ind = torch.where(pred == 1)  # [list of batch], [list of query ids]
    pred_positive = pred[pred_positive_ind]
    target_positive = target[pred_positive_ind]
    correct_precision = pred_positive.eq(target_positive)
    positive_num = pred_positive.shape[0]

    correct_precision = correct_precision.unsqueeze(0)
    res = []
    for k in topk:
        correct_k = correct_precision[:k].view(-1).float().sum(0)
        res.append(correct_k.mul_(100.0 / (positive_num + 1e-5)))
    return res