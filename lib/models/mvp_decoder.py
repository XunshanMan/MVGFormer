...
class MvPDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, query_pos, reference_points, src_views,
        ...
        # tgt_batch = []
        q = k = self.with_pos_embed(tgt, query_pos) ## add together;  Size is 1,150,256
        tgt2 = self.self_attn(q.transpose(0, 1),  ## transpose: swap dim0, dim1
                              k.transpose(0, 1),
        ...
        query_pos_expand = query_pos.unsqueeze(1).\
            expand((-1, nviews, -1, -1)).flatten(0, 1) ## 5,150,256
...
class MvPDecoder(nn.Module):
    def __init__(self, cfg, decoder_layer,
        ...
        if cfg.DECODER.share_layer_weights:
    ...
    ## Decoder: go through each layer, and update 3d pose
    def forward(self, tgt, reference_points, src_views,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output, reference_points = layer(output, query_pos, reference_points_input,
