...
from lib.utils.cameras import project_pose, projection_transform_batch, project_pose_batch
...
# 2D projection loss on the offsets
class PerProjectionL1Loss2D(nn.Module):
    def __init__(self, loss_type):
        super(PerProjectionL1Loss2D, self).__init__()
        self.loss_type = loss_type

    def forward(self, projection_pred, gt, cameras, center, scale,
                img_size, use_target_weight=False, target_weight=None):
        assert projection_pred.size(0) == gt.size(0)

        n_joints = gt.size(1)
        n_query, n_view, n_joint, _ = projection_pred.shape

        gt_multi_view = gt[:,None].repeat(1, n_view, 1, 1)
        
        projection_gt = project_pose_batch(gt_multi_view,cameras)
        weights_2d = torch.cat(
            [weight for weight in target_weight], 0)

        # pred size: people, views, joints, 2
        nviews = projection_pred.size(1)
        npeople = projection_pred.size(0)

        # Transform projection gt
        projection_gt = projection_transform_batch(projection_gt, center, scale, img_size, gt.device)  ## project ref/attention points
        # projection_gt = projection_gt.view(nviews, npeople, -1, 2)
        # projection_gt = projection_gt.transpose(0,1)

        projection_pred = projection_pred.reshape(-1, n_joints, 2)
        projection_gt = projection_gt.reshape(-1, n_joints, 2)
