...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        joints_3d_voxelpose_pred = db_rec.get('joints_3d_voxelpose_pred')
        ...
        joints_3d_voxelpose_pred_u = np.zeros((self.maximum_person, self.num_joints, 5))
        for i in range(nposes):
            joints_3d_u[i] = joints_3d[i][:, 0:3]
            joints_3d_vis_u[i] = joints_3d_vis[i][:, 0:3]
            if joints_3d_voxelpose_pred is not None:
                joints_3d_voxelpose_pred_u[i] = joints_3d_voxelpose_pred[i]
