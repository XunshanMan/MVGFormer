...
def projection_transform_batch(points, center, scale, img_size, device):
    trans_batch = []

    temp = []
    temp.append(
        torch.as_tensor(
            get_transform(center,
                            scale,
                            0, img_size),
            dtype=torch.float,
            device=device))
    ...
    points_trans = do_transform_batch(points, trans_batch)  ## project ref/attention points


    return points_trans

def project_ref_points(reference_points,
    cameras, 
    nviews,
    batch_size,
    nbins,
    ...
    reference_points_projected2d_xy = \
        project_pose_batch(reference_points_absolute, cam_batch)
...
def unfold_camera_param_batch(camera, device=None):
    ...
    # fix warning
    f = torch.stack([fx, fy], dim=2).float().to(device).view(bs, nview, 2, 1)
    c = torch.as_tensor(
...
def project_point_radial_batch(x_bs, R_bs, T_bs, f_bs, c_bs, k_bs, p_bs, deal_distortion=True):
    """
    Args
        x: Nx3 points in world coordinates
        R: 3x3 Camera rotation matrix
...
def project_pose_batch(x, camera, deal_distortion=True):
    R, T, f, c, k, p = unfold_camera_param_batch(camera, device=x.device)

    # Debug
    # print('Debug Projection: k=0, p=0')
    # k = torch.zeros(k.shape).to(k.device)
    # p = torch.zeros(p.shape).to(p.device)
    return project_point_radial_batch(x, R, T, f, c, k, p, deal_distortion=deal_distortion)
